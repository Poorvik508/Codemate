...
const ProfilePage = ({ isOwnProfile = true }) => {
  ...
  const [user, setUser] = useState({
    ...
    availability: "",
    skills: [],
    bio: "", // Corrected field name to match backend
  });
  const [newSkill, setNewSkill] = useState("");
  const [selectedPic, setSelectedPic] = useState(null);
  ...
  const fetchUserData = async () => {
    try {
      const res = await axios.get(backendUrl + "/api/profile/profile");
      setUser(res.data.user); // Now this works correctly since names match
    } catch (err) {
      console.error("Failed to fetch user data", err);
    }
  };
  ...
  const handleSave = async () => {
    try {
      const formData = new FormData();
      // Explicitly append all fields for reliability
      formData.append("name", user.name);
      formData.append("email", user.email);
      formData.append("college", user.college);
      formData.append("location", user.location);
      formData.append("availability", user.availability);
      formData.append("bio", user.bio); // Corrected this line to append `user.bio`

      if (selectedPic) {
        formData.append("profilePic", selectedPic);
      }

      const res = await axios.put(backendUrl + "/api/profile/profile", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });
