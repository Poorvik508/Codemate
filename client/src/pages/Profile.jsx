...
const ProfilePage = () => {
  ...
  const fetchUserData = async (userId) => {
    setIsLoading(true);
    try {
      // The backend now handles fetching the logged-in user if userId is not specified
      const url = userId
  ...
  useEffect(() => {
    if (id) {
      ...
      // Fetch the logged-in user's profile if userdata is available and there is no ID in the URL.
      fetchUserData(userdata._id);
    }
  }, [id, userdata]);

  const isOwnProfile = userdata && id === userdata._id;
  ...
  const addSkill = async () => {
    if (newSkill.trim() && user && !user.skills.includes(newSkill.trim())) {
      try {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-[#F9FAFB] w-full flex items-center justify-center">
